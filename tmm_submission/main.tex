...
Short-drama localization poses five coupled difficulties: (i) inconsistent punctuation complicating sentence boundaries and parsing~\cite{rehbein2020improving,donabauer2021making}; (ii) highly fragmented segments that omit subjects and referents, ambiguous without neighbouring context~\cite{li2021sentence,bawden2017evaluating}; (iii) brief on-screen subtitles requiring recognition robust to motion blur, clutter, and lighting~\cite{yan2020end,yu2025eve}; (iv) background music, overlapping speech, and accent variation that challenge ASR~\cite{liu2020adapting,yang2025speech}; and (v) dense character-specific proper nouns poorly covered by generic OCR/ASR vocabularies, whose errors propagate to translation.
...
Audiovisual translation is inherently a multimedia problem: subtitle output must satisfy joint visual, temporal, and linguistic constraints arising from different modalities of the same artefact~\cite{matusov2019challenges,karakanta2022mosub,tian2024mmsubtitle}.
...
Video-subtitle OCR has moved from template matching to deep models robust to varied fonts, backgrounds, and lighting~\cite{yan2020end}, and increasingly to vision--language models (VLMs): Qwen2-VL~\cite{wang2024qwen2} and its successor Qwen3-VL~\cite{qwen3vl2025}, InternVL2~\cite{chen2024internvl}, and GOT-OCR2.0~\cite{wang2024gotocr}.
Benchmarks confirm VLMs outperform traditional engines on video OCR while still struggling with temporal consistency and proper nouns~\cite{nagaonkar2025benchmarking,shi2025mme}; EVE~\cite{yu2025eve} targets end-to-end subtitle extraction.
...
End-to-end ASR can match HMM pipelines on broadcast media~\cite{liu2020modular}; video-guided post-correction refines ASR with visual cues~\cite{yang2025speech} (akin to our fusion but applied post-hoc), and punctuation/segmentation are key to subtitle readability~\cite{liu2020adapting}.

\subsection{Subtitle Translation}
NMT for subtitles has advanced with large language models~\cite{pramodya2025translating,wu2025combining,cui2025multilingual}, where context-aware translation~\cite{voita2018context,xiong2019modeling} and markup-aware sequence handling~\cite{cherry2021subtitle} matter for discourse coherence.
Qwen2.5~\cite{yang2024qwen2} supports Chinese--Japanese translation with efficient LoRA adaptation~\cite{hu2022lora}, and domain fine-tuning improves specialized-corpus quality~\cite{li2024eliciting}.
Dedicated MT models include NLLB-200~\cite{nllb2022}, M2M-100~\cite{fan2021beyond}, and SeamlessM4T-v2~\cite{seamless2023}; multimodal modelling of video~\cite{chen2023vast} further motivates our approach.
...
Neural TTS approaches near-human naturalness~\cite{shen2023naturalspeech}, and zero-shot voice cloning from short references is key for localization.
YourTTS~\cite{casanova2022yourtts}, GPT-SoVITS~\cite{gptsovits2024}, F5-TTS~\cite{chen2024f5tts}, CosyVoice2~\cite{cosyvoice2_2024}, and Voicebox~\cite{le2024voicebox} span VITS- and flow-matching-based cross-lingual synthesis; for reliability, API services such as EdgeTTS~\cite{edgetts2024} provide a robust fallback.
...
Table~\ref{tab:recognition} presents recognition performance; all OCR results use temporal deduplication (merging identical consecutive detections) to avoid inflated CER from repeated frames.
...
On perceived similarity GPT-SoVITS dominates (45\% of votes vs.\ 15\% F5-TTS, 5\% CosyVoice2; EdgeTTS 0\%); although F5-TTS has marginally higher \emph{automatic} similarity (0.729 vs.\ 0.722 in Table~\ref{tab:tts}), raters preferred GPT-SoVITS 3:1, showing embedding similarity alone misses perceptual identity, and the 35\% ``none'' votes reflect the difficulty of cross-lingual cloning from short references.
