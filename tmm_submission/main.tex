...
Fine-tuned \textbf{Qwen3-VL-4B} reaches BLEU $15.70\pm4.42$ (5-fold CV), $+25\%$ over its per-fold zero-shot mean (12.61; Table~\ref{tab:per_fold}), aided by full-episode context, an $r{=}32/\alpha{=}64$ LoRA, and a low learning rate; it is statistically comparable to zero-shot \textbf{Qwen3-8B} (per-fold mean 17.41, $p=0.453$) at half the parameters.
Fine-tuning \textbf{Qwen3-8B} reaches BLEU $19.69\pm2.49$ / COMET $0.860$ ($+2.28$ over its per-fold zero-shot mean), and fine-tuning the strongest baseline \textbf{Gemma-4-12B} reaches $30.75\pm1.84$ / chrF\texttt{++} $30.63$ ($+4.51$ over its per-fold zero-shot mean 26.24), the best overall---so domain LoRA fine-tuning helps at every scale (4B $+25\%$, 8B $+13\%$, 12B $+17\%$).
...
\begin{table}[t]
\centering
\caption{Per-fold BLEU on held-out validation episodes (5-fold CV, \texttt{strip\_nums}+\texttt{ja-mecab}, $n$\,=\,16/16/16/16/15). For each fold and for the mean, the best BLEU across all models (Gemma-4-12B FT) is shown in bold; the per-model FT-vs-ZS comparison is summarised in the footnote. FT+V denotes Qwen3-VL-4B fine-tuned with video frames as additional input.}
\label{tab:per_fold}
\resizebox{\columnwidth}{!}{%
...
He is the first author of this work.
...
She is the corresponding author of this work.
\end{IEEEbiographynophoto}
